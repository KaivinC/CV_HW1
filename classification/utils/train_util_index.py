...
def train(cfg,
    ...
    for epoch in range(start_epoch, epoch_num - 1):
        ...
        logging.info('current backbone lr: %f' %
        ...
        for batch_cnt, data in enumerate(data_loader['train']):
            ...
            N = imgs.size(0)
            positive_labels = labels.view(N, 1).expand(
                (N, num_positive)).contiguous().view(-1)
            optimizer.zero_grad()

            main_cls_feature, main_img_f, main_mask, coord_loss_1 = extractor(
                imgs)
            ...
            positive_cls_feature, positive_imgs_f, positive_masks, coord_loss_2 = extractor(
                positive_imgs)
            positive_probs = classifier(positive_cls_feature)
            cls_loss_2 = criterion(positive_probs, positive_labels)
            coord_loss_2 = torch.mean(
                coord_loss_2 * mask_to_binary(positive_masks))
            loss_2 = cls_loss_2 + coord_loss_2 * loss_weight['coord']

            n, c, h, w = positive_imgs_f.shape
            positive_imgs_f = positive_imgs_f.view(
                N,
                num_positive,
                c,
                h,
                ...
                h,
                w)
            positive_masks = positive_masks.view(
                N,
                num_positive,
                h,
                w).transpose(
            ...
            mask_reg_loss = calc_rela(
                all_img_features, all_pred_masks, num_positive).mean()
            ...
            batch_end = time.time()
            stt = '[TRAIN]: Epoch {:03d} / {:03d}    |    Batch {:03d} / {:03d}    |    Loss {:6.4f} + {:6.4f} + {:6.4f}*{:.2f} + {:6.4f}*{:.2f} + {:6.4f}*{:.2f} = {:6.4f}   |   cost {}'.format(
                epoch +
                1,
                epoch_num -
                1,
                ...
                1,
                L,
                cls_loss_1.data.item(),
                cls_loss_2.data.item(),
                coord_loss_1.data.item(),
                loss_weight['coord'],
                coord_loss_2.data.item(),
                loss_weight['coord'],
                mask_reg_loss,
                loss_weight['mask'],
                loss.data.item(),
        ...
        for batch_cnt_val, data_val in enumerate(data_loader['val']):
            ...
            logging.info(
                '[TEST]: Batch {:03d} / {:03d}'.format(batch_cnt_val + 1, LL))
        ...
        save_path = os.path.join(
            save_dir, 'weights-%d-%d-[%.4f].pth' %
            (epoch, batch_cnt, val_acc1))
        time.sleep(10)
