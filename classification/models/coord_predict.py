...
class RelativeCoordPredictor(nn.Module):
    def __init__(self, in_dim, size=7):
        ...
        self.basic_label = torch.from_numpy(
            self.build_basic_label(size)).float()
    ...
    def forward(self, x, mask):
        ...
        masked_x = masked_x.view(
            ...
            1,
            2).contiguous()  # (N, S, C)
        _, reduced_x_max_index = torch.max(
            torch.mean(masked_x, dim=-1), dim=-1)

        basic_index = torch.from_numpy(np.array([i for i in range(N)])).cuda()
        max_features = reduced_x[basic_index, reduced_x_max_index, :]  # (N, C)
        max_features_to_concat = max_features.unsqueeze(
            1).expand((N, H * W, C))

        discriminative_feature = torch.cat(
            (max_features_to_concat, reduced_x), dim=-1)  # (N, S, 2*C)
        ...
        label = label.unsqueeze(0).expand(
            (N, H, W, 2)).view(
            N, H * W, 2)  # (N, S, 2)
        basic_anchor = label[basic_index,
                             reduced_x_max_index,
                             :].unsqueeze(1)  # (N, 2)
        ...
        relative_angle = torch.atan2(
            relative_coord[:, :, 1], relative_coord[:, :, 0])  # (N, S) in (-pi, pi)
        relative_angle = (relative_angle / np.pi + 1) / 2  # (N, S) in (0, 1)

        # Calc Loss
        preds_dist, preds_angle = preds_coord[:,
                                              :, :, 0], preds_coord[:, :, :, 1]
